...
export default function HistoryPage() {
  ...
  return (
    <div className="min-h-dvh" style={{ backgroundColor: '#F8F4EF' }}>
      ...
      >
        <h1 className="text-xl font-black tracking-tight" style={{ color: '#1C1C1E' }}>記録一覧</h1>
        <div className="flex gap-2 mt-3">
          ...
          <span className="ml-auto text-xs self-center" style={{ color: '#8E8E93' }}>{filtered.length}件</span>
        </div>
      </header>
      ...
      <main className="px-5 pb-28 flex flex-col gap-6">
        {monthKeys.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-20">
            ...
            <p className="text-sm font-medium" style={{ color: '#8E8E93' }}>記録がありません</p>
          </div>
        ) : (
          monthKeys.map(key => {
            const monthSessions = grouped[key]
            const actualCount = monthSessions.filter(s => s.status === 'actual').length
            const totalRounds = monthSessions.filter(s => s.status === 'actual').reduce((sum, s) => sum + s.totalRounds, 0)
            return (
              <div key={key}>
                <div className="flex items-baseline gap-3 mb-3 px-1">
                  <h2 className="font-bold text-base" style={{ color: '#1C1C1E' }}>{formatMonthKey(key)}</h2>
                  {actualCount > 0 && (
                    <span className="text-xs" style={{ color: '#8E8E93' }}>{actualCount}回 · {totalRounds}R</span>
                  )}
                </div>
                <div className="flex flex-col gap-2">
                  {monthSessions.map(s => (
                    <SessionCard
                      key={s.id}
                      session={s}
                      onClick={() => router.push(`/u/${userId}/sessions/${s.id}`)}
                    />
                  ))}
                </div>
              </div>
      ...
      <div className="flex items-start justify-between">
        <div className="flex flex-col gap-1.5 flex-1">
          <div className="flex items-center gap-2">
            <span className="font-bold text-sm" style={{ color: '#1C1C1E' }}>{dateLabel}</span>
            <span
          ...
          <div className="flex items-center gap-3">
            <span className="text-2xl font-black" style={{ color: '#FF6B35' }}>{session.totalRounds}</span>
            <span className="text-sm font-medium" style={{ color: '#8E8E93' }}>ラウンド</span>
          </div>
          {session.roundBreakdown.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {session.roundBreakdown.map((b, i) => (
                <span key={i} className="text-xs px-2 py-1 rounded-lg font-medium" style={{ backgroundColor: '#F8F4EF', color: '#1C1C1E' }}>
          ...
          {session.trainerName && (
            <span className="text-xs" style={{ color: '#8E8E93' }}>👤 {session.trainerName}</span>
          )}
          {session.comment && (
        ...
        <div className="flex flex-col items-center gap-1 ml-3">
          {session.status === 'actual' && session.intensity && (
            <>
              ...
              <span className="text-xs font-medium" style={{ color: '#8E8E93' }}>{INTENSITY_LABELS[session.intensity]}</span>
            </>
          )}

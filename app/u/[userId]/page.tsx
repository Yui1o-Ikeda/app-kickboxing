...
export default function HomePage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const [all, appSettings] = await Promise.all([getSessions(userId), getSettings(userId)])
    ...
    <div className="min-h-dvh" style={{ backgroundColor: '#F8F4EF' }}>
      ...
      >
        <div>
          ...
          {isCurrentMonth && (
            <p className="text-xs" style={{ color: '#8E8E93' }}>今月の記録 · {userId}</p>
          )}
        </div>
        <button
          onClick={() => router.push(`/u/${userId}/sessions/new`)}

...
export default function Calendar({ year, month, sessions, onMonthChange, userId }: CalendarProps) {
  ...
  const handleDayClick = (day: number) => {
    ...
    if (existing && existing.length > 0) {
      router.push(`/u/${userId}/sessions/${existing[0].id}`)
    } else {
      router.push(`/u/${userId}/sessions/new?date=${dateStr}`)
    }
  }
      ...
      <div className="grid grid-cols-7 px-2 pb-4 gap-y-1">
        {days.map((day, idx) => {
          ...
          return (
            ...
            >
              <span
                className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium ${isToday ? 'text-white' : ''}`}
                style={{
                  backgroundColor: isToday ? '#FF6B35' : 'transparent',
                  color: isToday ? 'white' : dow === 0 ? '#FF3B30' : dow === 6 ? '#007AFF' : '#1C1C1E',
                }}
              >
                {day}
              </span>
              <div className="flex gap-0.5 mt-0.5 h-2">
                ...
                {hasPlanned && (
                  <span className="w-1.5 h-1.5 rounded-full border" style={{ borderColor: '#FF6B35' }} />
                )}
              </div>
            </button>
          )
